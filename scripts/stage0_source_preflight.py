...
def main() -> int:
    ...
    rows, _plan, summary = run(PreflightOptions(input_root=args.input_root.resolve(), report_dir=args.report_dir.resolve(), apply_renames=args.apply_renames, quarantine=args.quarantine, quarantine_root=args.quarantine_root.resolve(), include_unknown_quarantine=args.include_unknown_quarantine, limit=args.limit))
